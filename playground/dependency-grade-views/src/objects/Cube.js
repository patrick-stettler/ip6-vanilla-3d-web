...
/**
 * Converts a hex color string to a CSS rgb triplet string for use in rgb().
 * @param  {!string} hex - six-digit hex color, e.g. '#e05252'. Mandatory.
 * @return {string} space-separated RGB values, e.g. '224 82 82'
 */
 ...
 * The glass appearance is driven by the CSS variable --cl-c (RGB triplet).
 * @param  {!CubeConfigType} config - position and appearance of the cube. Mandatory.
 * @return {BoxType}
 * @example
 *     const cube = createCube({ name: 'Prog 1', color: '#FB8C00', x: 0, y: 0 });
 *     scene.add(cube);
 */
function createCube({ name, color, x, y }) {
  ...
  box.element.style.setProperty('--cl-c', hexToRgbTriplet(color));

  /** @type {HTMLSpanElement[]} */
  const labels = ['top', 'front', 'back', 'left', 'right'].map(function(face) {
  ...
  /**
   * Replaces the text shown on every labeled face (e.g. to show the grade
   * instead of the module name while the grade scale is active).
